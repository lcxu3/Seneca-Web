...
function observationsByGeoPrivacy(data, geoPrivacy) {
  if (geoPrivacy === undefined || typeof geoPrivacy === 'number' || typeof geoPrivacy === 'boolean')
...
item.geoprivacy && geoPrivacy && item.geoprivacy.toLowerCase() === geoPrivacy.toLowerCase() )
...
function getObservationsById(data, ...ids) {
  ans=[];
  ids.forEach((e)=>{
    if(data[e]!=null)ans.push(data[e])
  })
...
for (int i = 0; i < 10; i++)
{
/rnd.Next(100) + 1 numbers.Add(rnd.Next(100) + 1);
}
...
Console.WriteLine();
/
var updatedNumbers = numbers.Select(number => MapList(number)).ToList();

updatedNumbers.Sort();
foreach (var item in updatedNumbers)
{
Console.Write(item + " ");
}
Console.WriteLine();
...
public static int MapList(int value)
{
if (value > 50)
{
return value + 10;
